...
u8 Adc_u8ConvertChannel(u8 channel){
	//select channel
	ADMUX=(0xE0&ADMUX)|channel;
	//start conversion
	SET_BIT(ADCSRA,6);
	//wait unitl the conversion end
...
void ADC_vidInit(void)
{
...
#if Adc_u8DivisionFactorSelect==Adc_u8DivisionFactor8
...
#elif Adc_u8DivisionFactorSelect==Adc_u8DivisionFactor2
	...
	/*************to select the trigger edge for int0*********************/

#if Adc_u8AutoTriggerMode==Adc_u8EXTERNAL_INTERRUPT_REQUEST0
	CLEAR_BIT(SFIOR,7);
	SET_BIT(SFIOR,6);
	CLEAR_BIT(SFIOR,5);
#elif Adc_u8AutoTriggerMode==Adc_u8FREE_RUNNING_MODE
	CLEAR_BIT(SFIOR,7);
	CLEAR_BIT(SFIOR,6);
	CLEAR_BIT(SFIOR,5);
...
u16 ADC_u16ConvertChannel(u8 channel)
{
	...
	u16 ADC_Result=0;
	ADMUX=(ADMUX&0xE0)|channel;    //b7afez 3lla awel 3 high bit w el ba2y be asfar 3ashan a7afez 3lla ADMUX(7,6,5)
...
#elif Adc_u8Adjust==Adc_u8LeftAdjust
	ADC_Result=ADCH;    //MOVE LOW PART FOR CONVERSION TO RETURNING VALUE
#endif
	/*********************************************************/
